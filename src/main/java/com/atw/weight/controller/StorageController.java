...
public class StorageController {
	...
	public CommonResult saveInOutHistory(@RequestParam("inOutType") int inOutType, @RequestParam("num") Double num,
			@RequestParam("goods") String goods, @RequestParam("updateTime") String updateTime,
			HttpServletRequest request, HttpServletResponse response) {
		log.info("普通出入库操作");
		String memo = request.getParameter("memo");
		CommonResult commonResult = new CommonResult();
		Date updateTime1;
		try {
			updateTime1 = new SimpleDateFormat("yyyyMMddHHmmss").parse(updateTime);
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			commonResult.setStatus(1);
		...
		commonResult = storageService.saveInOutHisotry(inOutType, num, memo, goods, updateTime1);
		return commonResult;
	}
